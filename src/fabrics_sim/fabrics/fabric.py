...
class BaseFabric(torch.nn.Module):
    ...
    def __init__(self, device, batch_size, timestep, fabric_params_filename, fabric_params=None,
        ...
        self.joints_meta: list[FabricJointMeta] = []
    ...
    def allocate_scaled_accel(self):
        ...
        self.allocated_data['qdd_scaled'] = wp.zeros(shape=(self.batch_size, self.num_joints), device=self.device)
    ...
    def get_joint_names(self) -> list[str]:
        return [joint.name for joint in self.joints_meta]

    @property
    def num_joints(self):
        return len(self.joints_meta)
    ...
    def load_robot(self, robot_dir_name=None, robot_name=None, batch_size=None):
        ...
        if robot_dir_name is not None:
            ...
            # Load the robot.
            # NOTE: Keep the builder as member for more convenience browsing model info (than from the model itself).
            self.model_builder = newton.ModelBuilder()
            ...
            for i, joint_name in enumerate(self.model_builder.joint_key):
                ...
                joint_enabled = self.model_builder.joint_enabled[i]
                if joint_enabled and joint_type == newton.JointType.REVOLUTE:
                    joint_dof_id = self.model_builder.joint_qd_start[i]
                    self.joints_meta.append(FabricJointMeta(name=joint_name, type=joint_type,
                                                            enabled=joint_enabled,
                                                            dof_id=joint_dof_id,
                                                            upper_limit=self.model_builder.joint_limit_upper[joint_dof_id],
                                                            lower_limit=self.model_builder.joint_limit_lower[joint_dof_id]))
        ...
        self.root_metrics = torch.zeros(self.batch_size, self.num_joints, self.num_joints, device=self.device)
        self.root_geometric_forces = torch.zeros(self.batch_size, self.num_joints, device=self.device)
        self.root_potential_forces = torch.zeros(self.batch_size, self.num_joints, device=self.device)
        self.root_energy_metrics = torch.zeros(self.batch_size, self.num_joints, self.num_joints, device=self.device)
        self.root_energy_forces = torch.zeros(self.batch_size, self.num_joints, device=self.device)
        self.energies = torch.zeros(self.batch_size, 1, device=self.device)

        self.metric_inv = torch.zeros(self.batch_size, self.num_joints, self.num_joints, device=self.device)
        self.L = torch.zeros(self.batch_size, self.num_joints, self.num_joints, device=self.device)
        self.L_inv = torch.zeros(self.batch_size, self.num_joints, self.num_joints, device=self.device)
